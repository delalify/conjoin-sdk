...
type SuppressMessagesData = NonNullable<
  operations['suppressContactMessages']['responses']['200']['content']['application/json']['data']
>
...
type ListData = NonNullable<
  operations['listContacts']['responses']['200']['content']['application/json']['data']
>[number]

export function createMessagingContacts(client: ConjoinClient) {
  return {
    create: (data: CreateBody) => client.fetch<CreateData>('messaging/contacts/new', { method: 'POST', body: data }),

    suppressMessages: (contactId: string, data: SuppressMessagesBody) =>
      client.fetch<SuppressMessagesData>(`messaging/contacts/${contactId}/unsubscribe`, {
        method: 'PATCH',
        body: data,
      }),
    ...
    delete: (contactId: string) => client.fetch<DeleteData>(`messaging/contacts/${contactId}`, { method: 'DELETE' }),

    list: (data: ListBody) => client.fetchList<ListData>('messaging/contacts', { method: 'POST', body: data }),
  }
}
