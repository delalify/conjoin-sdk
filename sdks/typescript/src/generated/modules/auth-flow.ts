...
type CompleteSignupData = NonNullable<operations['completeSignup']['responses']['200']['content']['application/json']['data']>
...
type CompleteSigninData = NonNullable<operations['completeSignin']['responses']['200']['content']['application/json']['data']>
type StartPasswordResetBody = operations['startPasswordReset']['requestBody']['content']['application/json']
type StartPasswordResetData = NonNullable<operations['startPasswordReset']['responses']['200']['content']['application/json']['data']>
type CompletePasswordResetBody = operations['completePasswordReset']['requestBody']['content']['application/json']
type CompletePasswordResetData = NonNullable<operations['completePasswordReset']['responses']['200']['content']['application/json']['data']>
...
export function createAuthFlows(client: ConjoinClient) {
  return {
    ...
    completeSignup: (appId: string, clientId: string, data: CompleteSignupBody) =>
      client.fetch<CompleteSignupData>(`auth/flow/${appId}/${clientId}/signup/complete`, { method: 'POST', body: data }),
    ...
    completeSignin: (appId: string, clientId: string, data: CompleteSigninBody) =>
      client.fetch<CompleteSigninData>(`auth/flow/${appId}/${clientId}/signin/complete`, { method: 'POST', body: data }),

    startPasswordReset: (appId: string, clientId: string, data: StartPasswordResetBody) =>
      client.fetch<StartPasswordResetData>(`auth/flow/${appId}/${clientId}/password/reset/start`, { method: 'POST', body: data }),

    completePasswordReset: (appId: string, clientId: string, data: CompletePasswordResetBody) =>
      client.fetch<CompletePasswordResetData>(`auth/flow/${appId}/${clientId}/password/reset/complete`, { method: 'POST', body: data }),
  }
}
