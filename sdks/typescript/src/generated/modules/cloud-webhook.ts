...
type RegisterData = NonNullable<
  operations['registerCloudWebhook']['responses']['201']['content']['application/json']['data']
>
type ReadData = NonNullable<
  operations['readCloudWebhooks']['responses']['200']['content']['application/json']['data']
>[number]
...
type UpdateData = NonNullable<
  operations['updateCloudWebhook']['responses']['200']['content']['application/json']['data']
>
type DeleteData = NonNullable<
  operations['deleteCloudWebhook']['responses']['200']['content']['application/json']['data']
>
type ReadEventsData = NonNullable<
  operations['readCloudWebhookEvents']['responses']['200']['content']['application/json']['data']
>[number]
...
export function createCloudWebhooks(client: ConjoinClient) {
  return {
    ...
    delete: (webhookId: string) => client.fetch<DeleteData>(`cloud/cloud-webhook/${webhookId}`, { method: 'DELETE' }),

    readEvents: () => client.fetchList<ReadEventsData>('cloud/cloud-webhook/events'),
  }
}
