...
export function createAuthOrganizations(client: ConjoinClient) {
  return {
    create: (data: CreateBody) =>
      client.fetch<CreateData>('auth/organization/create', { method: 'POST', body: data }),

    read: (organizationId: string) =>
      client.fetch<ReadData>(`auth/organization/${organizationId}`),
    ...
    updateSettings: (organizationId: string, data: UpdateSettingsBody) =>
      client.fetch<UpdateSettingsData>(`auth/organization/${organizationId}/settings/update`, { method: 'PATCH', body: data }),
