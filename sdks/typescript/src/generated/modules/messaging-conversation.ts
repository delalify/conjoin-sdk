...
type SendMessageBody = operations['sendConversationMessage']['requestBody']['content']['application/json']
type SendMessageData = NonNullable<operations['sendConversationMessage']['responses']['201']['content']['application/json']['data']>
...
export function createMessagingConversations(client: ConjoinClient) {
  return {
    ...
    clone: (conversationId: string, data: CloneBody) =>
      client.fetch<CloneData>(`messaging/conversations/${conversationId}/clone`, { method: 'POST', body: data }),

    sendMessage: (conversationId: string, data: SendMessageBody) =>
      client.fetch<SendMessageData>(`messaging/conversations/${conversationId}/send`, { method: 'POST', body: data }),

    delete: (conversationId: string) =>
      client.fetch<DeleteData>(`messaging/conversations/${conversationId}`, { method: 'DELETE' }),
    ...
    archive: (conversationId: string, data: ArchiveBody) =>
      client.fetch<ArchiveData>(`messaging/conversations/${conversationId}/archive`, { method: 'PATCH', body: data }),
