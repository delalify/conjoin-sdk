...
type ListData = NonNullable<operations['listDiscounts']['responses']['200']['content']['application/json']['data']>[number]
...
export function createBillingDiscounts(client: ConjoinClient) {
  return {
    create: (data: CreateBody) =>
      client.fetch<CreateData>('billing/discount/create', { method: 'POST', body: data }),

    list: (query?: ListQuery) =>
      client.fetchList<ListData>('billing/discount/', { query: query as Record<string, unknown> }),

    delete: (discountId: string) =>
      client.fetch<DeleteData>(`billing/discount/${discountId}`, { method: 'DELETE' }),
  }
}
