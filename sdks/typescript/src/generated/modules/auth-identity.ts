...
type DiscoverAccountData = NonNullable<
  operations['discoverAccount']['responses']['200']['content']['application/json']['data']
>

export function createAuthIdentities(client: ConjoinClient) {
  return {
    link: (appId: string, accountId: string, data: LinkBody) =>
      client.fetch<LinkData>(`auth/identity/${appId}/account/${accountId}/identity/link`, {
        method: 'POST',
        body: data,
      }),

    unlink: (appId: string, accountId: string, data: UnlinkBody) =>
      client.fetch<UnlinkData>(`auth/identity/${appId}/account/${accountId}/identity/unlink`, {
        method: 'POST',
        body: data,
      }),
