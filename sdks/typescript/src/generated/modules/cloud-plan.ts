...
type ListData = NonNullable<
  operations['readCloudPlans']['responses']['200']['content']['application/json']['data']
>[number]
...
type ReadAccountData = NonNullable<
  operations['readAccountPlan']['responses']['200']['content']['application/json']['data']
>
...
export function createCloudPlans(client: ConjoinClient) {
  return {
    ...
    read: (slug: string) => client.fetch<ReadData>(`cloud/plan/single/${slug}`),

    readAccount: () => client.fetch<ReadAccountData>('cloud/plan/account'),
  }
}
