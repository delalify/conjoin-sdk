...
type SamlSpLogoutData = NonNullable<
  operations['samlSpLogout']['responses']['200']['content']['application/json']['data']
>
type SamlIdpLogoutBody = operations['samlIdpLogout']['requestBody']['content']['application/json']
type SamlIdpLogoutData = NonNullable<
  operations['samlIdpLogout']['responses']['200']['content']['application/json']['data']
>
...
export function createAuthSLOs(client: ConjoinClient) {
  return {
    ...
    samlSpLogout: (appId: string, accountId: string, data: SamlSpLogoutBody) =>
      client.fetch<SamlSpLogoutData>(`auth/slo/${appId}/account/${accountId}/slo/saml/sp-initiated`, {
        method: 'POST',
        body: data,
      }),
