...
type CreateData = NonNullable<
  operations['createPriceBundle']['responses']['201']['content']['application/json']['data']
>
type ListData = NonNullable<
  operations['listPriceBundles']['responses']['200']['content']['application/json']['data']
>[number]
...
type UpdateData = NonNullable<
  operations['updatePriceBundle']['responses']['200']['content']['application/json']['data']
>
type ArchiveData = NonNullable<
  operations['archivePriceBundle']['responses']['200']['content']['application/json']['data']
>
type RestoreData = NonNullable<
  operations['restorePriceBundle']['responses']['200']['content']['application/json']['data']
>
type ActivateBody = operations['activatePriceBundle']['requestBody']['content']['application/json']
type ActivateData = NonNullable<
  operations['activatePriceBundle']['responses']['201']['content']['application/json']['data']
>
type ReadActivationsData = NonNullable<
  operations['readPriceBundleActivations']['responses']['200']['content']['application/json']['data']
>[number]
...
type DeactivateData = NonNullable<
  operations['deactivatePriceBundle']['responses']['200']['content']['application/json']['data']
>
...
export function createBillingPriceBundles(client: ConjoinClient) {
  return {
    ...
    list: (entityId: string, referenceId: string, query?: ListQuery) =>
      client.fetchList<ListData>(`billing/price-bundle/${entityId}/${referenceId}`, {
    ...
    activate: (entityId: string, referenceId: string, data: ActivateBody) =>
      client.fetch<ActivateData>(`billing/price-bundle/${entityId}/activate/${referenceId}`, {
        method: 'POST',
        body: data,
      }),

    readActivations: (entityId: string, customerId: string, query?: ReadActivationsQuery) =>
      client.fetchList<ReadActivationsData>(`billing/price-bundle/${entityId}/activations/${customerId}`, {
        query: query as Record<string, unknown>,
      }),

    deactivate: (entityId: string, activationId: string, data: DeactivateBody) =>
      client.fetch<DeactivateData>(`billing/price-bundle/${entityId}/deactivate/${activationId}`, {
        method: 'POST',
        body: data,
      }),
  }
