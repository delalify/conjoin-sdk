...
type ReadSmsMessagesData = NonNullable<
  operations['readSmsMessages']['responses']['200']['content']['application/json']['data']
>[number]
type GetSmsSummaryBody = operations['getSmsSummary']['requestBody']['content']['application/json']
type GetSmsSummaryData = NonNullable<
  operations['getSmsSummary']['responses']['200']['content']['application/json']['data']
>

export function createMessagingSMs(client: ConjoinClient) {
  return {
    sendSms: (data: SendSmsBody) => client.fetch<SendSmsData>('messaging/sms/send', { method: 'POST', body: data }),
