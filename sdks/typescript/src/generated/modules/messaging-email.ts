...
type ReadMessagesData = NonNullable<
  operations['readEmailMessages']['responses']['200']['content']['application/json']['data']
>[number]
type GetSummaryBody = operations['getEmailSummary']['requestBody']['content']['application/json']
type GetSummaryData = NonNullable<
  operations['getEmailSummary']['responses']['200']['content']['application/json']['data']
>

export function createMessagingEmails(client: ConjoinClient) {
  return {
    send: (data: SendBody) => client.fetch<SendData>('messaging/email/send', { method: 'POST', body: data }),
    ...
    getSummary: (data: GetSummaryBody) => client.fetch<GetSummaryData>('messaging/email/summary', { body: data }),
  }
}
