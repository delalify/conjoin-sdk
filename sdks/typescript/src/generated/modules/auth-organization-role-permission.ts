...
type AssignOrgRolePermissionData = NonNullable<
  operations['assignOrgRolePermission']['responses']['200']['content']['application/json']['data']
>
type ListOrgRolePermissionsData = NonNullable<
  operations['listOrgRolePermissions']['responses']['200']['content']['application/json']['data']
>[number]
...
export function createAuthOrganizationRolePermissions(client: ConjoinClient) {
  return {
    assignOrgRolePermission: (organizationId: string, roleId: string, data: AssignOrgRolePermissionBody) =>
      client.fetch<AssignOrgRolePermissionData>(
        `auth/organization/role/permission/${organizationId}/${roleId}/assign`,
        { method: 'POST', body: data },
      ),

    listOrgRolePermissions: (organizationId: string, roleId: string, query?: ListOrgRolePermissionsQuery) =>
      client.fetchList<ListOrgRolePermissionsData>(`auth/organization/role/permission/${organizationId}/${roleId}`, {
