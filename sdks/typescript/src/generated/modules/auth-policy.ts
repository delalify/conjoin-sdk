...
type ListData = NonNullable<
  operations['listPolicies']['responses']['200']['content']['application/json']['data']
>[number]
...
export function createAuthPolicies(client: ConjoinClient) {
  return {
    create: (appId: string, organizationId: string, data: CreateBody) =>
      client.fetch<CreateData>(`auth/organization/policy/${appId}/organization/${organizationId}/policy/create`, {
        method: 'POST',
        body: data,
      }),
    ...
    list: (appId: string, organizationId: string, query?: ListQuery) =>
      client.fetchList<ListData>(`auth/organization/policy/${appId}/organization/${organizationId}/policy`, {
        query: query as Record<string, unknown>,
      }),

    update: (appId: string, organizationId: string, policyId: string, data: UpdateBody) =>
      client.fetch<UpdateData>(
        `auth/organization/policy/${appId}/organization/${organizationId}/policy/${policyId}/update`,
        { method: 'PATCH', body: data },
      ),

    delete: (appId: string, organizationId: string, policyId: string) =>
      client.fetch<DeleteData>(
        `auth/organization/policy/${appId}/organization/${organizationId}/policy/${policyId}/delete`,
        { method: 'POST' },
      ),
  }
}
