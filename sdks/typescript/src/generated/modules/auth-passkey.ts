...
type ListData = NonNullable<
  operations['listPasskeys']['responses']['200']['content']['application/json']['data']
>[number]
...
export function createAuthPasskeys(client: ConjoinClient) {
  return {
    ...
    delete: (appId: string, accountId: string, credentialId: string) =>
      client.fetch<DeleteData>(`auth/passkey/${appId}/account/${accountId}/passkeys/${credentialId}/delete`, {
        method: 'DELETE',
      }),
  }
}
