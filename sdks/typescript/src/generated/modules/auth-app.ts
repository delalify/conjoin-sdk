...
type ListData = NonNullable<
  operations['listAuthApps']['responses']['200']['content']['application/json']['data']
>[number]
...
type UpdateSettingsData = NonNullable<
  operations['updateAuthAppSettings']['responses']['200']['content']['application/json']['data']
>
type UpdateBrandingBody = operations['updateAuthAppBranding']['requestBody']['content']['application/json']
type UpdateBrandingData = NonNullable<
  operations['updateAuthAppBranding']['responses']['200']['content']['application/json']['data']
>
type UpdateCustomAttributesBody =
  operations['updateAuthAppCustomAttributes']['requestBody']['content']['application/json']
type UpdateCustomAttributesData = NonNullable<
  operations['updateAuthAppCustomAttributes']['responses']['200']['content']['application/json']['data']
>

export function createAuthApps(client: ConjoinClient) {
  return {
    create: (data: CreateBody) => client.fetch<CreateData>('auth/app/create', { method: 'POST', body: data }),

    read: (appId: string) => client.fetch<ReadData>(`auth/app/${appId}`),

    list: (query?: ListQuery) => client.fetchList<ListData>('auth/app/', { query: query as Record<string, unknown> }),
    ...
    updateCustomAttributes: (appId: string, data: UpdateCustomAttributesBody) =>
      client.fetch<UpdateCustomAttributesData>(`auth/app/${appId}/settings/custom-attributes-schema`, {
        method: 'PUT',
        body: data,
      }),
  }
